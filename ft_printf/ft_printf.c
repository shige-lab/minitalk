...
/*   Updated: 2021/11/07 21:44:34 by tshigena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

size_t	select_format(char c, va_list ap);
int		is_overflow(int str_len, size_t printed_len);
...
size_t	select_format(char c, va_list ap)
{
	...
	if (c == 'c')
		...
		size += print_p(va_arg(ap, unsigned long));
	else if (c == 'x' || c == 'X')
