...
/*   Updated: 2021/11/08 23:34:08 by tshigena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_itoa_dx(unsigned long long n, int base)
{
	...
	digit = nummber_of_digits(n, base);
	if (base == 2)
		str_num = (char *)ft_calloc(7, sizeof(char));
	else
		str_num = (char *)ft_calloc(digit + 1, sizeof(char));
	if (!str_num)
		return (NULL);
	if (digit == 6 && base == 2)
		str_num[0] = '0';
